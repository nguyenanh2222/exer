...
def solve(input_data):
    ...
    for diction in input_data:
        # print(diction)
        if diction['name'][0] == 'H':
            data_update_1 = dict()
            data_update_1['name'] = diction['name']
            data_update_1['population'] = diction['population']
            rs_1.append(data_update_1)

    rs_old_1 = list(filter(lambda x: x['population'] > 1_000_000, rs_1))
    print(sorted(rs_old_1, key=lambda x: x["population"]))
    print(list(sorted(rs_1, key=lambda x: slugify(x["name"]))))

    rs_2 = list()

    for diction in input_data:
        data_update_2 = dict()
        data_update_2 = diction
        rs_2.append(data_update_2)

    rs_old_2 = list(filter(lambda x: x['population'] > 1_000_000, rs_2))
    print(sorted(rs_old_2, key=lambda x: x["population"]))
